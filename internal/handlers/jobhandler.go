...
func (h *Handler) ProcessJobApp(c *gin.Context) {
	ctx := c.Request.Context()
	traceId, ok := ctx.Value(middleware.TraceIdKey).(string)
	if !ok {
		log.Error().Msg("traceId missing from context")
	...
	var requestData []models.RequestJob //store the data in dummmy struct
	...
	for _, v := range requestData {
		...
		//requestData = append(requestData, v)
	}
	job, err := h.service.ProcessJob(requestData)
	if job == nil {
		log.Error().Err(err).Str("Trace Id", traceId).Msg("problem in checking job details")
		c.AbortWithStatusJSON(http.StatusBadRequest, gin.H{"msg": "data is not same"})
		return
	}
