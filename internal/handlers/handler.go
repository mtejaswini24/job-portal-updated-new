...
func API(a *auth.Auth, sc repository.UserRepo, rdb cache.Caching) *gin.Engine {
	r := gin.New() //create a new engine
	m, err := middleware.NewMid(a)
	if err != nil {
		log.Panic().Msg("middlewares not set up")
	}
	ms, err := services.NewService(sc, rdb)
	if err != nil {
	...
	r.POST("api/job/compare", m.Authenticate(h.ProcessJobApp))
